...
function num(val) {
    val = val.toString(); // converting to string so it won'm added together in display

    if(cal_done)// if calculation is done
    cls();  // clear everything

    if(!operPressed) { // if no operator is pressed (means it's first value)
        if(!val_1)
        val_1 = 0;
    ...
    if(operPressed) {// if operator is pressed (means it's 2nd value)
        if(!val_2)
        val_2 = 0;
